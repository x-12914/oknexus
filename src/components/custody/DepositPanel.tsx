...
export function DepositPanel() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((e) => {
        if (!cancelled) setAddrErr((e as Error).message);
      });
    return () => {
      cancelled = true;
    };
  ...
  return (
    <div className="p-6 max-w-lg mx-auto">
      ...
      <h1 className="text-xl font-semibold mb-1">Deposit crypto</h1>
      <p className="text-sm text-[var(--color-muted)] mb-4">
        {config
          ? `${chainLabel(config.chain)} · ${config.minConfirmations} confirmations to credit`
          : "…"}
      </p>
      ...
      <div className="rounded-lg border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-xs text-amber-500 mb-4 flex gap-2">
        ...
        Testnet only — send test coins on {config ? chainLabel(config.chain) : "the test network"}.
        Never send real mainnet assets to this address.
      </div>
            ...
            >
              ...
              <div className="text-right">
                ...
                >
                  {d.status === "CREDITED"
                    ? "Credited"
                    : `Confirming (${d.confirmations}/${config?.minConfirmations ?? "?"})`}
                </div>
                <a
                  href={d.explorerUrl}
                  target="_blank"
